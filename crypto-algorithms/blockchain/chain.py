...
if __name__ == "__main__":
    ...
    chain.add_transaction(Transaction("Bob", "Charlie", 0.5, 0.1, 1))
    ...
    for i in range(3):
        ...
        if block:
            print(f"Block {i+1} mined: {block.get_hash()}")
    ...
    # Demonstrate fork handling
    fork_block = Block(version=1, previous_hash=chain.main_chain[1])
    fork_block.add_transaction(Transaction("Dave", "Eve", 2.0, 0.2, 1))
    fork_block.mine(chain.difficulty)
    
    print("\nAdding fork block...")
    if chain.add_block(fork_block):
        print("Fork block added")
        print(f"Main chain tip: {chain.main_chain[-1]}")
        print(f"Fork blocks: {stats['fork_blocks']}")
    
    # Check balances
    print("\nBalances:")
    addresses = ["Alice", "Bob", "Charlie", "miner"]
